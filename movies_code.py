...
def get_movies_by_title(title):
    ...
    return d
token=jwt.encode({'name' : "webapp", 'exp' : datetime.datetime.utcnow() + datetime.timedelta(minutes=30)}, app.config['SECRET_KEY']) 
...
def token_required(f,a=token):
    ...
    def decorated(*args, **kwargs):
        ...
        try: 
            data = jwt.decode(a, app.config['SECRET_KEY'])
